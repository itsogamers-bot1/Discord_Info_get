...
UTC = timezone.utc
BASE_DIR = os.getcwd()
VOLUNTARY_LEAVES_FILE = os.path.join(BASE_DIR, 'voluntary_leaves.csv')
ROLE_STATS_FILE = os.path.join(BASE_DIR, 'role_stats.csv')
SERVER_STATS_FILE = os.path.join(BASE_DIR, 'server_stats.csv')
...
MONITOR_GUILD_ID = os.getenv('MONITOR_GUILD_ID')
OUTPUT_GUILD_ID = os.getenv('OUTPUT_GUILD_ID')
...
def write_to_sheet(sheet_name: str, values: List[List[Any]], range_name: Optional[str] = None, headers: Optional[List[str]] = None) -> bool:
    ...
    if not SPREADSHEET_ID:
        ...
        return False
    return write_to_sheet_general(SPREADSHEET_ID, sheet_name, values, range_name, headers=headers)
...
def write_to_spreadsheet(username: str, status: str, error_message: str = "", current_roles: str = "") -> bool:
    ...
    try:
        ...
        range_name = f'{ONBOARDING_SHEET_NAME}!A:E'
        body = {
            'values': [row]
        }
    ...
    try:
        ...
        if os.path.exists(VOLUNTARY_LEAVES_FILE):
            try:
                ...
                logger.error(f"voluntary_leaves.csvの読み込み中にエラーが発生しました: {e}")
...
async def process_stats(ctx=None):
    ...
    try:
        ...
        if ctx:
            ...
            output_guild_id = int(os.getenv('OUTPUT_GUILD_ID', '0'))
...
async def on_member_update(before: discord.Member, after: discord.Member):
    ...
    try:
        # サーバーID確認
        server_id = os.getenv("SERVER_ID")
        if server_id and (not after.guild or str(after.guild.id) != str(server_id)):
            return
...
async def write_voluntary_leaves_to_sheet() -> bool:
    ...
    if not SHEETS_ENABLED or not VOLUNTARY_LEAVES_SHEET_ID or not os.path.exists(VOLUNTARY_LEAVES_FILE):
        return False
    ...
    sheet_name = "退会者統計"
    return write_to_sheet_general(VOLUNTARY_LEAVES_SHEET_ID, sheet_name, data_rows, headers=headers)
...
async def on_ready():
    ...
    print('\n=== 接続情報 ===')
    monitor_guild = bot.get_guild(int(os.getenv('MONITOR_GUILD_ID', '0')))
    if monitor_guild:
        print(f'  - 名前: {monitor_guild.name}')
        print(f'  - メンバー数: {monitor_guild.member_count}')
    
    output_guild = bot.get_guild(int(os.getenv('OUTPUT_GUILD_ID', '0')))
    if output_guild:
        channel = output_guild.get_channel(int(os.getenv('OUTPUT_CHANNEL_ID', '0')))
...
if __name__ == "__main__":
    ...
    server_id = os.getenv("SERVER_ID")
    if not server_id:
        print("警告: SERVER_IDが設定されていません。すべてのサーバーのメッセージを処理します。")
